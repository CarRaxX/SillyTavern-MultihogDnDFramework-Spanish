...
- Do not emit a map command merely because the party enters, owns, rents, visits, shops in, or sleeps in an ordinary house, shop, inn, headquarters, warehouse, or other mundane building. The only exception is when {{user}} explicitly asks for a persistent map of that exact named building or explicitly asks for their recurring home/base layout to remain persistent. Then map that building once with kind DUNGEON; here DUNGEON means room-scale, not dangerous. Use threat NONE when no actual danger is established; LOW means light but real danger.
...
- Do not emit a map command for a district, alley, house, shop, rooftop, warehouse, or other sub-place of a settlement unless the explicit standalone-building exception above applies. If the party is already inside an unmapped city or town, map that city or town — not the incidental place where they are standing. Inventing or entering a granular location does not create a new map.
...
- Use these exact field names: site (the exact footer location root), entrance, kind (DUNGEON or SETTLEMENT), scale (SMALL, MEDIUM, or LARGE), threat (NONE, LOW, MODERATE, HIGH, or DEADLY), premise. Do not invent keys such as name or footer_root.
- Scale is geographic size. Threat is site danger (enemy/trap density), never party level. NONE has no active danger; LOW has light but real danger. A LARGE LOW ruin can be vast with one localized danger; a SMALL DEADLY vault can be a meat grinder.
[CREATE_AREA_MAP]
