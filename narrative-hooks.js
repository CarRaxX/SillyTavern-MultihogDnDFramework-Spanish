...
export function registerMapArchitectTool() {
    try {
        ...
        registerFunctionTool({
            ...
            description: 'Creates and saves the complete private objective map for a site before its first exploration. Call exactly once when the player enters an unmapped dungeon/ruin/lair (kind DUNGEON, room-scale) or an unmapped town/city/village as a whole (kind SETTLEMENT, district-scale). Do not call merely for entering or owning an ordinary building. A named house, shop, inn, headquarters, or other building may use DUNGEON only when the player explicitly requests that exact building as a persistent room-scale map or recurring home/base. Do not call for wilderness, roads, countryside, or other places between mapped sites. SETTLEMENT site is the city/town name, not the current alley. Do not call if [MAPPED_SITES] lists that site or a parent of the place being entered, even when no DUNGEON_REALITY block is attached yet. Do not call if a DUNGEON_REALITY block already supplies the site map. The dedicated architect validates all routes and assets, writes the map to the root Location entry, and returns compact private canon for narration.',
            parameters: {
                type: 'object',
                properties: {
                    site: { type: 'string', description: 'For SETTLEMENT, copy the town/city/village name from the Location footer, not the current street, alley, or interior. For DUNGEON, copy the dungeon/ruin name or the exact named standalone building explicitly requested by the player. Copy character-for-character. Never translate, transliterate, expand, or retitle. A distinct site title must already appear in the footer.' },
                    entrance: { type: 'string', description: 'Copy the named entrance exactly as printed: a settlement gate/square/docks, dungeon door, or explicitly requested standalone-building entrance. Never translate it. This becomes the first VISITED map area.' },
                    kind: { type: 'string', enum: ['DUNGEON', 'SETTLEMENT'], description: 'DUNGEON = room-scale interior: normally a high-risk ruin/lair/stronghold, or an ordinary named building only when the player explicitly requests that exact building as a persistent map. SETTLEMENT = the city/town/village as a whole, district-scale — not an alley or incidental building. Required.' },
                    ...
                    threat: { type: 'string', enum: ['NONE', 'LOW', 'MODERATE', 'HIGH', 'DEADLY'], description: 'Site danger for occupancy and trap density. Independent of party level and scale. NONE = no active danger may be invented. LOW = light but real danger. MODERATE = some hostiles, HIGH = frequent hostiles and traps, DEADLY = layered overlapping threats. Use NONE for a peaceful home/base unless actual danger is established.' },
