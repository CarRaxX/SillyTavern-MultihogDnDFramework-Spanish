...
/**
 * Player-facing map prose for Adventure Companion: visited/discovered areas,
 * unrevealed neighbors as Unexplored, and only suspected/known assets.
 */
export function formatDungeonMapForPlayer(documentOrContent, currentLocation = '') {
    ...
    const areasById = new Map(document.areas.map(area => [area.id, area]));
    const visibleAreas = document.areas.filter(area =>
    ...
    const areaLabel = (id) => (visibleIds.has(id) ? (areasById.get(id)?.name || id) : 'Unexplored');
    ...
    if (placement.area) {
        const interior = placement.interiorAsset?.name || placement.unmatchedInterior || '';
        lines.push(interior
    ...
    for (const area of visibleAreas) {
        for (const connection of area.connections || []) {
            ...
            if (seen.has(key)) continue;
            seen.add(key);
            const detail = visibleIds.has(connection.to) && connection.detail
                ? ` — ${connection.detail}`
                : '';
    ...
    for (const area of visibleAreas) {
        ...
        if (area.knowledge === 'VISITED') {
            ...
            lines.push('- Not yet entered.');
        }
        const assets = document.assets.filter(asset =>
            asset.location === area.id
            && String(asset.knowledge || '').toUpperCase() !== 'UNREVEALED');
        if (assets.length) {
