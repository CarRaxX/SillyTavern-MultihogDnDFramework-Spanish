...
- Start the map block with \`Dungeon Site: [top-level site label]\`, exactly matching the first location segment in the status footer.
- Give every mapped area a unique, short natural-language label within the site (for example \`Area: Guard Post - East\`). Use qualifiers to disambiguate similar areas; never use opaque room IDs or coordinates.
- Under those stable labels, describe connected areas, entrances, exits, elevation, doors, hazards, traps, secret features, creatures, patrol routes, alarms, light, sound, and environmental conditions. Reuse the labels in later narration when the party moves or a fact changes.
...
- Do not emit follow-up map chunks or \`data-dungeon-delta\` blocks. After the initial map, narrate durable changes normally; the Lorebook Agent owns persistent room/area updates under the site's Location entries.
