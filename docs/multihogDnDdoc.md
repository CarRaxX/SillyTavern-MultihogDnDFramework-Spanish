...
The Adventure Companion cannot flip the Components checkbox or open Visuals/Map itself. Tell the player where those controls are. If they enable **Inject current site map** in CHAT options, the Companion receives the player-facing map (visited/discovered areas, unexplored neighbors as unknown) to discuss exploration.
