...
const MAPS_GUIDE_HTML = `
    <div style="padding:24px;max-width:760px;max-height:calc(85vh - 48px);box-sizing:border-box;overflow-y:auto;text-align:left;font-family:var(--rt-font,system-ui,sans-serif);color:var(--SmartThemeBodyColor,inherit);line-height:1.55;">
        ...
        <p>Maps are only injected in context when you’re inside the mapped location. This is to prevent the context from exploding, as maps can be quite large JSON objects. The intended design is that you carry the events back in terms of normal Lorebook Agent entries after you leave the map, so what happened inside is carried in memory. It should not be necessary to have the full map in context at all times.</p>
